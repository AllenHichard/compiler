...
public class ArquivoSemantico {
    ...
    public ArquivoSemantico() throws FileNotFoundException {
    ...
    private void retirarArquivosSaidas() {
        for (int i = 0; i < arquivos.length; i++) {
            if (arquivos[i].toString().contains("lexico - ") && !arquivos[i].toString().contains("Semantico - ")) {
    ...
    public void SalvarArquivo(List tokens, boolean erro) throws IOException {
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter("entrada\\" + "Semantico - " + a[1]));
        for (int i = 0; i < tokens.size(); i++) {
            writer.write(tokens.get(i).toString() + "\r\n");
        }
        if(erro){
            writer.write("\r\n\r\n Código Semanticamente correto - Sucesso");
        }
        writer.close();
        lerArq.close();
        prox++;
